...
export default function FAQFormSheet({ isOpen, onClose, onSuccess, faqId }: any) {
  ...
  useEffect(() => {
    const loadKategori = async () => {
      try {
        ...
        const uniqueKategori = [...new Set(data.map((item: any) => item.kategori).filter(Boolean))];
  ...
  const handleChange = (e: any) => {
    ...
    if (name === "kategori" && value === "new") {
      ...
      setForm((prev: any) => ({ ...prev, kategori: "" }));
    } else {
      setForm((prev: any) => ({ ...prev, [name]: value }));
    }
  };

  const handleNewKategoriChange = (e: any) => {
  ...
  return (
    <Sheet open={isOpen} onOpenChange={(open: any) => !open && onClose()}>
