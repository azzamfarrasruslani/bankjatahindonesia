...
import { fetchArtikel } from "@/features/artikel";
...
export default function ArtikelGrid() {
  ...
  const highlightArtikel = artikelList
    .filter((a: any) => a.is_top)
    .sort((a: any, b: any) => new Date(b.created_at) - new Date(a.created_at))
    .slice(0, 3);
  const otherArtikel = artikelList.filter((a: any) => !highlightArtikel.includes(a));
          ...
          >
            ...
            {highlightArtikel.length > 0 && (
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {highlightArtikel.map((item: any, index: any) => (
                    <ArtikelHighlight
                      key={item.id}
                      id={item.id}
            ...
            {otherArtikel.length > 0 && (
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-10">
                  {otherArtikel.map((item: any, i: any) => (
                    <ArtikelCard
                      key={item.id}
                      id={item.id}
                      image={item.gambar_url}
