...
import { fetchLokasi } from "@/features/lokasi";
import { LokasiFilter, LokasiList, LokasiMap, LokasiSkeleton } from "@/features/lokasi";
import HeroSection from "@/components/common/HeroSection";
import { motion, AnimatePresence } from "framer-motion";

const MapContainer = dynamic(
  () => import("react-leaflet").then((m: any) => m.MapContainer),
  { ssr: false },
);
const TileLayer = dynamic(
  () => import("react-leaflet").then((m: any) => m.TileLayer),
  { ssr: false },
);
const Marker = dynamic(() => import("react-leaflet").then((m: any) => m.Marker), {
  ssr: false,
});
const Popup = dynamic(() => import("react-leaflet").then((m: any) => m.Popup), {
  ssr: false,
});
...
export default function LokasiPage() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      import("leaflet").then((leaflet: any) => setLeaflet(leaflet));
  ...
  const filteredData = useMemo(() => {
    return lokasiData.filter((lokasi: any) => {
      const cocokJenis =
