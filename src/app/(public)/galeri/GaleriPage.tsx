...
export default function GaleriPage() {
      ...
      try {
        ...
        if (data && data.length > 0) {
          const uniqueCategories = [
            "Semua",
            ...new Set(data.map((item: any) => item.kategori).filter(Boolean)),
          ];
  ...
  const filteredGaleri =
    filterKategori === "Semua"
      ? galeri
      : galeri.filter((item: any) => item.kategori === filterKategori);
  ...
  return (
    <main className="min-h-screen bg-white pb-24">
      ...
      <section className="relative z-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-10 lg:-mt-16">
        ...
        >
          {kategoriList.map((kat: any, idx: any) => (
            <button
              key={idx}
        ...
        {loading && (
          <div className="columns-1 sm:columns-2 lg:columns-3 gap-6 space-y-6">
            {Array.from({ length: 9 }).map((_: any, i: any) => (
              <div
                key={i}
        ...
        {!loading && filteredGaleri.length > 0 && (
          <div className="columns-1 sm:columns-2 lg:columns-3 gap-6 space-y-6 pb-20">
            {filteredGaleri.map((item: any, i: any) => (
              <motion.div
                key={item.id}
                initial={{ opacity: 0, y: 30 }}
                animate={{ opacity: 1, y: 0 }}
        ...
        {selectedImage && (
          ...
          >
            ...
            <motion.div
              ...
              onClick={(e: any) => e.stopPropagation()} // Prevent close when clicking content
            >
