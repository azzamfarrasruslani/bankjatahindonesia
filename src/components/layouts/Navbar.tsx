...
export default function Navbar() {
  ...
  const toggleDropdown = (key: any) => {
    setOpenDropdown((prev: any) => (prev === key ? null : key));
  };
  ...
  return (
    <motion.nav
      className={`fixed top-4 left-1/2 z-50 w-[95%] max-w-7xl transform -translate-x-1/2 rounded-2xl backdrop-blur-md border transition-all duration-500 ${
        isScrolled
          ? "bg-white/95 shadow-2xl border-gray-200/80 py-1"
          : "bg-white/90 shadow-lg border-gray-100 py-2"
      }`}
                      ...
                      >
                        <div className="ml-4 space-y-1 mt-2">
                          <MobileNavItem
                            ...
                            label="Tabungan Jelantah"
                            onClick={() => setIsOpen(false)}
                            subItem
                          />
                          <MobileNavItem
                            href="/program/jual-beli-jelantah"
                            label="Jual Beli Jelantah"
                            onClick={() => setIsOpen(false)}
                            subItem
                          />
                          <MobileNavItem
                            href="/program/sedekah-jelantah"
                            label="Sedekah Jelantah"
                            onClick={() => setIsOpen(false)}
                            subItem
                          />
                        </div>
...
const MobileNavItem = ({ href, label, onClick, subItem = false }: any) => (
