...
export default function HeroSection({ title, description, imageUrl }: any) {
  const pathname = usePathname();
  const ref = useRef(null);

  // Generate breadcrumbs from pathname
  const pathSegments = pathname.split("/").filter((segment: any) => segment !== "");
  const breadcrumbs = pathSegments.map((segment: any, index: any) => {
      ...
      >
        ...
        >
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-[4.5rem] font-bold leading-[1.1] tracking-tight text-white drop-shadow-2xl">
            {title.split(" ").map((word: any, i: any) => (
              <span
                key={i}
                className={
